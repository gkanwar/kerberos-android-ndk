    ...
    char *name;
{
    ...
    while ((code = krb5_kt_next_entry(kcontext, kt, &entry, &cursor)) == 0) {
        ...
        log("%s", pname);
        if (show_etype)
            log(" (%s) " , etype_string(entry.key.enctype));
        if (show_keys) {
            log(" (0x");
            {
                unsigned int i;
                for (i = 0; i < entry.key.length; i++)
                    log("%02x", entry.key.contents[i]);
            }
            log(")");
        }
        log("\n");
    ...
    krb5_address *a;
{
    ...
    case ADDRTYPE_INET:
        if (a->length != 4) {
        broken:
            log("broken address (type %d length %d)",
                    a->addrtype, a->length);
            return;
        }
        {
...
#endif
    default:
        log("unknown addrtype %d", a->addrtype);
        return;
    }
    ...
    if (err) {
        log("unprintable address (type %d, error %d %s)", a->addrtype, err,
                gai_strerror (err));
        return;
    }
    log("%s", namebuf);
